...
class CPUTest extends GameMode {
  ...
  draw(ctx) {
    ...
    if (!this.cached_hint) {
      ...
      hintCtx.fillText('Click to increase load | TEST RUNS FOR 2 MINUTES | E to stop', 400, 35);
...
class RAMTest extends GameMode {
  ...
  draw(ctx) {
    ...
    if (!this.cached_hint) {
      ...
      hintCtx.fillStyle = rgbToCSS(COLORS.WHITE);
      hintCtx.textAlign = 'center';
      hintCtx.fillText('Click to increase load | TEST RUNS FOR 2 MINUTES | E to stop', 450, 35);
      this.cached_hint = hintCanvas;
    }
